...
class Grenade:
    ...
    def try_contact_explode(self, targets: list["Stickman"]) -> bool:
        """Detonate when the grenade body touches a non-owner stickman.

        Player grenades pop on enemy contact; enemy grenades pop on player contact.

        Args:
            targets: all stickmen in the scene.

        Returns:
            True if contact triggered an explosion this frame.
        """
        ...
        for target in targets:
            if not target.alive or id(target) == self.owner_id:
                continue
            hit_rect = target.rect.inflate(c.GRENADE_CONTACT_PAD, c.GRENADE_CONTACT_PAD)
            if hit_rect.collidepoint(int(self.x), int(self.y)):
                self._explode()
                return True
        return False
...
class AK47Pickup:
    ...
    def try_collect(self, player: "Player") -> bool:
        ...
        cy = self.y + bob
        dist = math.hypot(player.x - self.x, player.y - cy - 20)
        if dist > c.CHEST_PICKUP_RADIUS + c.STICK_W:
            return False
    ...
    def draw(self, surf: pygame.Surface) -> None:
        ...
        for i in range(3):
            r = int(c.CHEST_PICKUP_RADIUS + 8 + i * 7 + pulse * 5)
            alpha = int(85 - i * 22)
            glow = pygame.Surface((r * 2, r * 2), pygame.SRCALPHA)
        ...
        for i in range(5):
            ...
            sx = cx + math.cos(ang) * (c.CHEST_PICKUP_RADIUS + 5)
            sy = cy + math.sin(ang) * (c.CHEST_PICKUP_RADIUS + 5)
...
def chest_reward_label(reward_type: str) -> str:
    """Return a HUD-friendly label for a chest reward type.

    Args:
        reward_type: key from config.CHEST_REWARD_TYPES.

    Returns:
        Short display string for toast / UI.
    """
    if reward_type == "bow":
        return f"Bow x{c.BOW_ARROWS_PER_SET}"
    if reward_type == "ak47":
        return f"AK-47 x{c.AK47_ROUNDS_PER_PICKUP}"
    if reward_type == "grenades":
        return f"+{c.CHEST_GRENADE_BONUS} Grenades"
    if reward_type in c.WEAPONS:
        return c.WEAPONS[reward_type]["label"]
    return reward_type.capitalize()


def apply_chest_reward(player: "Player", reward_type: str) -> str:
    """Grant one chest reward to the player.

    Args:
        player: human stickman collecting the chest.
        reward_type: pre-rolled reward key.

    Returns:
        Display label for HUD toast.
    """
    if reward_type == "bow":
        player.equip_bow(c.BOW_ARROWS_PER_SET)
    elif reward_type == "ak47":
        player.equip_ak47(c.AK47_ROUNDS_PER_PICKUP)
    elif reward_type == "grenades":
        player.add_grenades(c.CHEST_GRENADE_BONUS)
    elif reward_type in ("sword", "hammer", "pickaxe"):
        player.equip_melee_weapon(reward_type)
    return chest_reward_label(reward_type)


# ---------------------------------------------------------------------------
# ChestPickup — mystery loot chest, one random reward per spawn
# ---------------------------------------------------------------------------

class ChestPickup:
    """World pickup: wooden chest with a single pre-rolled reward."""

    def __init__(self, x: float, y: float, reward_type: str) -> None:
        """Args:
            x: centre x on the arena.
            y: feet-level y (chest floats above).
            reward_type: reward granted on collection (rolled at spawn).
        """
        self.x = x
        self.y = y - 32
    ...
    def update(self) -> None:
        """Animate golden glow and bob."""
        self._shine_phase += 0.11
        self._bob_phase += 0.07

    def try_collect(self, player: "Player") -> bool:
        """Open chest and grant its reward when the player touches it.
        ...
        if dist > c.CHEST_PICKUP_RADIUS + c.STICK_W:
            return False
        apply_chest_reward(player, self.reward_type)
        self.active = False
        return True

    def draw(self, surf: pygame.Surface) -> None:
        """Render mystery chest with golden trim and question mark.
        ...
        # Golden glow rings
        for i in range(3):
            r = int(c.CHEST_PICKUP_RADIUS + 6 + i * 8 + pulse * 5)
            alpha = int(80 - i * 22)
            glow = pygame.Surface((r * 2, r * 2), pygame.SRCALPHA)
            pygame.draw.circle(glow, (*c.CHEST_GLOW, alpha), (r, r), r)
            surf.blit(glow, glow.get_rect(center=(cx, cy)))

        # Chest body
        body = pygame.Rect(cx - 22, cy - 10, 44, 28)
        lid = pygame.Rect(cx - 24, cy - 22, 48, 14)
        pygame.draw.rect(surf, c.CHEST_WOOD, body, border_radius=3)
        pygame.draw.rect(surf, c.CHEST_DARK, lid, border_radius=4)
        pygame.draw.rect(surf, c.CHEST_GOLD, body, width=2, border_radius=3)
        pygame.draw.rect(surf, c.CHEST_GOLD, lid, width=2, border_radius=4)
        # Lock plate
        pygame.draw.rect(surf, c.CHEST_GOLD, (cx - 5, cy - 4, 10, 12))
        pygame.draw.circle(surf, c.CHEST_DARK, (cx, cy + 2), 3)

        # Sparkles
        for i in range(4):
            ang = self._shine_phase + i * (math.tau / 4)
            sx = cx + math.cos(ang) * (c.CHEST_PICKUP_RADIUS + 4)
            sy = cy + math.sin(ang) * (c.CHEST_PICKUP_RADIUS + 4)
            pygame.draw.circle(surf, c.CHEST_GOLD, _ip(sx, sy), 2)

        font = pygame.font.SysFont("Arial", 22, bold=True)
        qmark = font.render("?", True, c.CHEST_GOLD)
        surf.blit(qmark, qmark.get_rect(center=(cx, cy - 16)))

        label_font = pygame.font.SysFont("Arial", 15, bold=True)
        label = label_font.render("Loot Chest", True, c.WHITE)
        surf.blit(label, label.get_rect(midbottom=(cx, cy - 34)))
...
class Player(Stickman):
    ...
    def add_grenades(self, count: int) -> None:
        """Add grenades up to the per-match cap.

        Args:
            count: grenades to add.
        """
        self.grenades_left = min(c.GRENADES_PER_MATCH, self.grenades_left + count)
