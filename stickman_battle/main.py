...
class GameScene:
    ...
    def __init__(self, difficulty: str) -> None:
        ...
        # ---- Loot chest & rifle effects ----
        self._chest: Optional[ChestPickup] = None
        self._chest_spawn_timer = 0
        self._chest_toast = ""
        self._chest_toast_frames = 0
    ...
    def _random_pickup_spot(self) -> tuple[float, float]:
        """Pick a random arena position for pickups and chests.

        Returns:
            (x, y) feet-level coordinates.
        """
        floor_y = c.SCREEN_H - 20
        spots = [
    ...
    def _spawn_bow_pickup(self) -> None:
        ...
        x, y = self._random_pickup_spot()
        self._bow_pickup = BowPickup(x, y)

    def _spawn_chest(self) -> None:
        """Spawn or replace the mystery loot chest with one random reward."""
        x, y = self._random_pickup_spot()
        reward = random.choice(c.CHEST_REWARD_TYPES)
        self._chest = ChestPickup(x, y, reward)
    ...
    def update(self) -> Optional["GameOverScene"]:
        ...
        # Recurring loot chest every 20 seconds (replaces any uncollected chest)
        self._chest_spawn_timer += 1
        if self._chest_spawn_timer >= c.CHEST_SPAWN_INTERVAL_F:
            self._chest_spawn_timer = 0
            self._spawn_chest()

        if self._chest_toast_frames > 0:
            self._chest_toast_frames -= 1
        ...
        if self._chest is not None:
            self._chest.update()
            if self._chest.try_collect(self._player):
                self._chest_toast = chest_reward_label(self._chest.reward_type)
                self._chest_toast_frames = c.CHEST_TOAST_FRAMES
                self._chest = None
            elif not self._chest.active:
                self._chest = None
        ...
        all_stickmen = [self._player] + self._enemies
        for grenade in self._grenades:
            grenade.update(self._platforms)
            if not grenade.exploded:
                grenade.try_contact_explode(all_stickmen)
            if grenade.just_exploded:
                hits = grenade.apply_explosion(all_stickmen)
    ...
    def draw(self, surf: pygame.Surface) -> None:
        ...
        # Loot chest
        if self._chest is not None and self._chest.active:
            self._chest.draw(surf)
    ...
    def _draw_hud(self, surf: pygame.Surface) -> None:
        ...
        # Loot chest hint
        if self._chest is not None and self._chest.active:
            chest_hint = self._font_sm.render(
                "Chest nearby! Walk into it to open.",
                True, (255, 200, 80),
            )
            surf.blit(chest_hint, chest_hint.get_rect(midtop=(c.SCREEN_W // 2, 58)))

        # Reward toast after opening a chest
        if self._chest_toast_frames > 0 and self._chest_toast:
            toast = self._font_sm.render(
                f"Got: {self._chest_toast}",
                True, (255, 230, 120),
            )
            surf.blit(toast, toast.get_rect(midtop=(c.SCREEN_W // 2, 80)))
