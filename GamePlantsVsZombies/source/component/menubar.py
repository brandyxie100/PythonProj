"""
Plants vs Zombies - Menubar Component
=====================================
Plant card selection panel: static, moving, and bowling modes.
"""
...
_SUN_FONT: pg.font.Font | None = None


def getSunValueImage(sun_value):
    """Render the sun counter digits (cached font for smoother frames)."""
    global _SUN_FONT
    if _SUN_FONT is None:
        _SUN_FONT = pg.font.SysFont(None, 22)
    font = _SUN_FONT
    width = 32
...
class Card():
    ...
    def setSelect(self, can_select):
        ...
        if can_select:
            ...
            self.image.fill((255, 255, 255, 128), special_flags=pg.BLEND_RGBA_MULT)

    def setFrozenTime(self, current_time):
        self.frozen_timer = current_time

    def createShowImage(self, sun_value, current_time):
        """Create a card image for cooldown or insufficient-sun status."""
        time = current_time - self.frozen_timer
        if time < self.frozen_time:  # cool down status
            image = pg.Surface((self.rect.w, self.rect.h), pg.SRCALPHA)
            frozen_image = self.orig_image.copy()
            frozen_image.fill((255, 255, 255, 128), special_flags=pg.BLEND_RGBA_MULT)
            ...
            image.fill((255, 255, 255, 192), special_flags=pg.BLEND_RGBA_MULT)
        else:
...
class MenuBar():
    ...
    def loadFrame(self, name):
        ...
        # Keep a clean copy so sun digits do not ghost when redrawn each frame.
        self._bg_image = self.image.copy()
    ...
    def drawSunValue(self):
        # Restore the bar background before blitting digits to avoid trails.
        self.image.blit(self._bg_image, (0, 0))
...
class Panel():
    ...
    def loadImages(self, sun_value):
        ...
        self.menu_rect.x = 0
        self.menu_rect.y = 0
        self._menu_bg = self.menu_image.copy()
    ...
    def draw(self, surface):
        self.menu_image.blit(self._menu_bg, (0, 0))
...
class MoveCard():
    ...
    def createShowImage(self):
        """Create a partial card image while the card slides in from the right."""
...
class MoveBar():
    ...
    def deleateCard(self, card):
        """Deprecated alias for deleteCard (kept for older call sites)."""
        self.deleteCard(card)

    def deleteCard(self, card):
        """Remove a consumed card from the move/bowling bar."""
